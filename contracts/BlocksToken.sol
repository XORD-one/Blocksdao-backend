...
contract BlocksToken is IBlocksToken, Ownable, ERC721Checkpointable {
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        require(_exists(tokenId), "blocksToken: URI query for nonexistent token");
     ...
     * @dev Only callable by the owner when not locked.
     */
    function setSeeder(IBlocksDaoSeeder _seeder) external override onlyOwner whenSeederNotLocked {
        seeder = _seeder;
    ...
    function _mintTo(address to, uint256 blockId) internal returns (uint256) {
        IBlocksDaoSeeder.Seed memory seed = seeds[blockId] = seeder.generateSeed(blockId, descriptor);
