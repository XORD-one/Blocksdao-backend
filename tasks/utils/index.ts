...
export async function getGasPriceWithPrompt(ethers: typeof ethersType & HardhatEthersHelpers): Promise<BigNumber> {
...
export async function printEstimatedCost(factory: ContractFactory, gasPrice: BigNumber) {
  const deploymentGas = await factory.signer.estimateGas(factory.getDeployTransaction({ gasPrice }));
  const deploymentCost = deploymentGas.mul(gasPrice);
  console.log(`Estimated cost to deploy NounsDAOLogicV2: ${utils.formatUnits(deploymentCost, "ether")} ETH`);
}
...
export function dataToDescriptorInput(data: string[]): {
  ...
  const encodedCompressed = `0x${deflateRawSync(Buffer.from(abiEncoded.substring(2), "hex")).toString("hex")}`;
